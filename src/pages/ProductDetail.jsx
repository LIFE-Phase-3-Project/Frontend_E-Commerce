...
const ProductDetail = () => {
  ...
  const images = productDetailItem?.image?.map(image => ({
    original: image,
    thumbnail: image
  })) || [];
      ...
      <section className="container flex-grow mx-auto max-w-[1200px] border-b py-5 lg:grid lg:grid-cols-2 lg:py-10">
        ...
        <div className="mx-auto px-5 lg:px-5 dark:text-cream">
          ...
          <div className="mt-1">
            <div className="flex items-center">
              <Rater
                ...
                total={5}
                interactive={true}
                rating={productDetailItem?.ratings || 0}
              />
              <p className="ml-3 text-sm text-gray-400">
                ({productDetailItem?.reviews?.length || 0})
              </p>
            </div>
          </div>
          ...
          <div className="mt-7 flex flex-row items-center gap-6">
            <button
              className="flex h-12 w-1/3 min-w-36 items-center justify-center bg-custom-purple text-white duration-100 hover:bg-on-hover-purple"
              onClick={() => addToCart(productDetailItem)}
            >
              <FontAwesomeIcon icon={faTruck} />
              {t("addToCart")}
            </button>
            <button className="flex h-12 w-1/3 min-w-36 items-center justify-center bg-light-pink duration-100 hover:bg-on-hover-pink">
