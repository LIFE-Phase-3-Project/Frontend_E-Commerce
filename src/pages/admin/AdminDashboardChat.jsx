      ...
      <div className="flex flex-col lg:flex-row gap-6 h-full">
        <div className="lg:w-1/3 bg-white dark:bg-admin-sidebar-color shadow-lg rounded-lg p-4 lg:p-6 my-5 lg:my-6 flex-shrink-0">
          ...
          <ul className="space-y-4">
            {rooms.map((room) => (
              <li key={room.id} className="flex justify-between items-center p-3 bg-gray-200 dark:bg-gray-400 rounded-lg shadow-sm hover:bg-gray-300 transition">
                <span className="text-sm lg:text-lg font-medium">{`Room name || ${room.id}`}</span>
                <button 
